  ...
  const getLeaderboard = async () => {
    try {
      ...
      if (!waitlistData || waitlistData.length === 0) {
        console.warn('No waitlist entries found');
        return [];
      }
      
      console.log(`Retrieved ${waitlistData.length} waitlist entries`);
      
      // Then get referral counts from the referrals table
      ...
      if (referralError) {
        ...
        // If referrals table query fails, return waitlist entries with zero referrals
        const basicLeaderboard = waitlistData.map((entry: any) => ({
          ...entry,
          referral_count: 0,
          joined_at: entry.created_at ? new Date(entry.created_at).toLocaleDateString() : 'Unknown',
        }));
        
        console.log('Returning waitlist entries with zero referrals due to referral query error');
